...
<head>
  ...
  <title>Add Teacher - Bright Future School</title>
...
<body>
    ...
    <div class="container">
      <div class="section-header" style="margin-bottom: 3rem;">
        ...
        <h1 class="section-title">
          Add New
          <span class="gradient">Teacher</span>
        </h1>
      ...
      <!-- Back Navigation -->
      <a href="teacher.php" style="display: inline-flex; align-items: center; gap: 0.5rem; color: var(--color-primary); text-decoration: none; margin-bottom: 1.5rem; font-size: 0.9rem; transition: opacity 0.2s;" onmouseover="this.style.opacity='0.7'" onmouseout="this.style.opacity='1'">
      ...
      <div class="card" style="max-width: 600px; margin: 0 auto; padding: 2rem;">
        <form id="addTeacherForm" action="../../api/teachers.php?action=create" method="POST" style="display: flex; flex-direction: column; gap: 1.5rem; width: 100%;">
          <div class="grid-2">
          ...
          <div class="form-group">
            <label for="email" style="display: block; margin-bottom: 0.5rem; color: white; font-weight: 500;">Email Address</label>
            <input 
              type="email" 
              id="email" 
              name="email" 
          ...
          <div class="form-group">
            <label for="subjects" style="display: block; margin-bottom: 0.5rem; color: white; font-weight: 500;">Subjects (Comma Separated)</label>
            <input 
              type="text" 
              id="subjects" 
              name="subjects" 
          ...
          <div class="form-group">
            <label for="gradeId" style="display: block; margin-bottom: 0.5rem; color: white; font-weight: 500;">Assigned Grade</label>
            <select 
              id="gradeId" 
              name="gradeId" 
              class="form-control"
  ...
  <script>
    ...
    // Teacher form submission handling
    document.addEventListener('DOMContentLoaded', function() {
      ...
      form.addEventListener('submit', function(e) {
        ...
        submitBtn.textContent = 'Saving Teacher...';
        submitBtn.disabled = true;
        
        // Prepare form data
        const formData = new FormData(form);
        
        // Convert FormData to object and handle grade mapping
        const formDataObj = {};
        for (let [key, value] of formData.entries()) {
          // Map form field names to API field names
          let fieldName = key.replace(/([A-Z])/g, '_$1').toLowerCase();
          
          // Special handling for gradeId - need to get grade name
          if (fieldName === 'grade_id') {
            ...
            if (selectedOption.value !== '') {
              formDataObj['grade'] = selectedOption.text; // Use the text (grade name)
            }
          } else {
        ...
        fetch('../../api/teachers.php?action=create', {
          method: 'POST',
          headers: {
        ...
        .then(data => {
          if (data.success) {
            ...
            const message = `Teacher added successfully! Default password: ${data.default_password}`;
            showNotification(message, 'success');
            
            // Reset form after successful submission
            form.reset();
          } else {
            showNotification(data.message || 'Error adding teacher', 'error');
          }
        })
        .catch(error => {
