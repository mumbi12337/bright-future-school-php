...
<head>
  ...
  <title>Grades - Bright Future School</title>
...
<body>
    ...
    <div class="container">
      <div class="section-header" style="margin-bottom: 3rem;">
        ...
        <p class="section-description">
          View, edit, and manage student grades
        </p>
      </div>
      ...
      <div class="card" style="margin-bottom: 2rem; padding: 1.5rem;">
        <div class="header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; gap: 10px; flex-wrap: wrap;">
          <div style="display: flex; align-items: center; gap: 1rem;">
            <input type="text" class="form-group" placeholder="Search grades..." style="padding: 0.75rem 1rem; border-radius: 12px; background: rgba(255, 255, 255, 0.05); border: 1px solid var(--color-border); color: white; font-size: 1rem; width: 250px;">
          </div>
        ...
        <table style="width: 100%; border-collapse: collapse;">
          <thead>
            <tr style="background-color: var(--color-surface);">
              <th style="padding: 1rem; text-align: left; border-bottom: 1px solid var(--color-border);">Grade Name</th>
              <th style="padding: 1rem; text-align: left; border-bottom: 1px solid var(--color-border);">Students</th>
              <th style="padding: 1rem; text-align: left; border-bottom: 1px solid var(--color-border);">Exams</th>
              <th style="padding: 1rem; text-align: left; border-bottom: 1px solid var(--color-border);">Fees</th>
              <th style="padding: 1rem; text-align: left; border-bottom: 1px solid var(--color-border);">Notes</th>
          ...
          <tbody id="grades-tbody">
            ...
            <tr style="border-bottom: 1px solid var(--color-border);">
              <td style="padding: 1rem; color: white; font-weight: 500;"><?= htmlspecialchars($grade['name']) ?></td>
              <td style="padding: 1rem; color: var(--color-text);"><?= $grade['student_count'] ?></td>
              <td style="padding: 1rem; color: var(--color-text);"><?= $grade['exam_count'] ?></td>
              <td style="padding: 1rem; color: var(--color-text);"><?= $grade['fee_count'] ?></td>
              <td style="padding: 1rem; color: var(--color-muted);"><?= htmlspecialchars($grade['notes'] ?? '') ?></td>
              <td style="padding: 1rem;">
                <div class="actions" style="display: flex; gap: 0.75rem;">
                  <a href="view.php?id=<?= $grade['id'] ?>" class="btn btn-primary" style="padding: 0.5rem 1rem; font-size: 0.875rem; text-decoration: none; min-width: auto;">View</a>
                  <button class="btn btn-secondary delete-grade" data-id="<?= $grade['id'] ?>" style="padding: 0.5rem 1rem; font-size: 0.875rem; min-width: auto; background: rgba(220, 38, 38, 0.2); border: 1px solid #dc2626; color: #ef4444;">
                    Delete
                  </button>
                </div>
              </td>
            ...
            <?php if (empty($grades)): ?>
            <tr>
              <td colspan="6" style="padding: 1rem; text-align: center; color: var(--color-text);">No grade levels found</td>
            </tr>
            <?php endif; ?>
          </tbody>
        </table>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      console.log('Grades management loaded');
          ...
          if (confirm('Are you sure you want to delete this grade?')) {
            ...
            .then(data => {
              if (data.success) {
                ...
                showNotification('Error deleting grade: ' + (data.message || 'Unknown error'), 'error');
              }
            })
            .catch(error => {
              showNotification('Error deleting grade: ' + error.message, 'error');
            });
          }
        });
      });
